...
const Slider = () => {
  ...
  const ButtonGroup = ({ next, previous, goToSlide, ...rest }) => {
    ...
    return (
      <div className="">
        <div
          className={`${
            currentSlide === 0 ? "disable hidden" : ""
          }absolute h-full top-0 left-0 w-[2rem] bg-[rgba(0,0,0,0.5)] `}
          onClick={() => previous()}
        ></div>
        <div
          className="absolute h-full top-0 right-0 w-[2rem] bg-[rgba(0,0,0,0.5)]"
          onClick={() => next()}
        ></div>
        <div className="absolute h-[1.2rem] w-full flex justify-center items-center gap-1 left-0 bottom-0">
          {Array.from({ length: imageSrcArray.length - 2 }).map((_, index) => (
            <div
  ...
  return (
    <Carousel
      ...
      draggable={false}
    >
      {imageSrcArray.map((img, index) => (
        <div className="px-1" key={"carousel" + index}>
