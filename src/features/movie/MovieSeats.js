...
const MovieSeats = ({
  ...
  const displaySeatLayOut = () => {
    return Object.keys(seatLayOut).map((key, index) => {
      if (key.startsWith("Hall")) {
        return (
          <div key={"seat hall" + index} className="flex-1 p-[0.5rem]"></div>
        );
      }
      let seatCount = 0;
      return (
        ...
        >
          ...
          {seatLayOut[key].map((sit) => {
            let seatAmount = parseInt(sit, 10); // Parse the string as an integer with base 10
            let seatType = sit.charAt(sit.length - 1); // Get the last character of the string
            if (seatType === "H") {
  ...
  const SeatComponent = ({ seatNumber, seatType }) => {
    ...
    if (selectedSeats.includes(seatNumber)) {
      ...
      seatClass +=
        " bg-emerald-700 border border-emerald-400 hover:bg-cyan-500";
    }
